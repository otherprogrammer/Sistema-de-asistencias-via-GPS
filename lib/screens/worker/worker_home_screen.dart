...
class _WorkerHomeScreenState extends State<WorkerHomeScreen>
  ...
  Future<void> _checkTodayAttendanceStatus() async {
    ...
    try {
      ...
      if (user != null) {
        ...
        bool hasCheckOut =
            await _attendanceService.hasCheckedOutToday(user.uid);
  ...
  void _startAutoSync() {
    _connectivitySubscription =
        _offlineSync.watchConnectivity().listen((results) async {
                          ...
                          children: [
                            Expanded(
                              child: _buildActionButton(
                                ...
                                isLoading: _isProcessingLocation &&
                                    !_hasCheckedInToday,
                              ),
                            ),
                            const SizedBox(width: 16),
                            Expanded(
                              child: _buildActionButton(
                                ...
                                label: _hasCheckedOutToday
                                    ? 'Salida\nRegistrada'
                                    : 'Marcar\nSalida',
                                icon: _hasCheckedOutToday
                                    ? Icons.check_circle
                                    : Icons.logout,
      ...
      child: Row(
        ...
        children: [
          _buildStatusIndicator(
              'Entrada', _hasCheckedInToday, AppColors.success),
  ...
  Widget _buildStatusIndicator(
      String label, bool isCompleted, Color activeColor) {
    return Row(
      children: [
        AnimatedContainer(
  ...
  Future<void> _handleMarkAttendance(BuildContext context, String type) async {
    ...
    try {
      ...
      FaceVerificationResult verificationResult =
          await _faceRecognitionService.verifyFace(
        userId: user.uid,
  ...
  void _showSuccessDialog(
      BuildContext context, String type, String attendanceId) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 6),
                    _buildInfoRow(Icons.my_location, 'Precisión',
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            const SizedBox(width: 12),
            const Expanded(
                child: Text('Sincronización Completa',
                    style: TextStyle(fontSize: 18))),
          ],
        ),
        content: Text(
  ...
  void _showFaceVerificationFailedDialog(
      BuildContext context, double similarity, String type) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: Row(
          children: [
            ...
            const Expanded(
              child:
                  Text('Verificación Fallida', style: TextStyle(fontSize: 18)),
            ),
          ],
        ),
        content: Column(
